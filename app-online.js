...
function legacy_BASE_show(name){
 ...
 if(name==='jutsus')renderJutsuPage();
...
function masteryBattleHtml(v,stage){const tier=Number(v.tier||1),turns=tier===1?7:6;return `<div class="masteryBattleBrief"><div class="masteryBattleSeal">戦</div><p>Vença o sparring em até <b>${turns} turnos</b> e mantenha <b>${esc(char(v.character)?.name||v.character)}</b> consciente. Uma vitória lenta ou com o usuário derrotado não aprova a técnica.</p><button id="masteryBattleStart">INICIAR COMBATE DE DOMÍNIO</button></div>`}
...
function show(name){
 ...
 if(name==='home')renderHome();if(name==='jutsus')renderJutsuPage();if(name==='tasks')renderMissions();if(name==='ninjaMissions')renderNinjaMissions();if(name==='events')renderEvents();if(name==='profile')renderProfile();if(name==='shop')renderShop();if(name==='inventory')renderInventory();if(name==='encyclopedia')renderEncyclopedia();if(name==='online')renderOnline();if(name==='story')renderStory();if(name==='select')renderRoster()
}
