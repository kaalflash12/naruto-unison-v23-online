...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(route==='/room'&&req.method==='POST'){
      const b=await body(req),code=String(b.code||'').trim().toUpperCase().replace(/[^A-Z0-9_-]/g,'').slice(0,24);if(!code)return json({ok:false,error:'Código de sala obrigatório.'},400);const {data:old}=await db.from('sns_rooms').select('revision,owner_account_id').eq('code',code).maybeSingle();if(old?.owner_account_id&&old.owner_account_id!==account.id)return json({ok:false,error:'Sala pertence a outra conta.'},403);const rev=Number(old?.revision||0)+1;const {data,error}=await db.from('sns_rooms').upsert({code,owner_account_id:account.id,state:b.state||{},revision:rev,updated_at:new Date().toISOString()}).select('code,state,revision,updated_at').single();if(error)throw error;const {error:memberError}=await db.from('sns_room_members').upsert({room_code:code,account_id:account.id,last_seen_at:new Date().toISOString()},{onConflict:'room_code,account_id'});if(memberError)throw memberError;return json({ok:true,room:data});
    }
    if(route==='/room'&&req.method==='GET'){
      const code=String(u.searchParams.get('code')||'').trim().toUpperCase();const {data,error}=await db.from('sns_rooms').select('code,state,revision,updated_at,owner_account_id').eq('code',code).maybeSingle();if(error)throw error;if(!data)return json({ok:true,room:null});if(data.owner_account_id!==account.id){const {data:membership,error:memberError}=await db.from('sns_room_members').select('account_id').eq('room_code',code).eq('account_id',account.id).maybeSingle();if(memberError)throw memberError;if(!membership)return json({ok:false,error:'Entre na sala antes de acessar este recurso.'},403)}const {owner_account_id,...room}=data;return json({ok:true,room});
    }
