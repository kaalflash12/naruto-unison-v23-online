...
const RESERVED=new Set([
  "userId","username","role","level","xp","ryo","rank","items","equipment","equipmentSlots",
...
async function roleFor(db:any,userId:string){const {data,error}=await db.from("admin_principals").select("admin_principal_roles(role)").eq("user_id",userId).eq("active",true).maybeSingle();if(error)return "PLAYER";return String(data?.admin_principal_roles?.[0]?.role||"PLAYER")}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(req.method==="GET"){
      const [baseRes,latest,role]=await Promise.all([load(db,u.id),db.from("player_save_snapshots").select("revision,schema_version,snapshot,created_at").eq("user_id",u.id).order("revision",{ascending:false}).limit(1).maybeSingle(),roleFor(db,u.id)]);
      if(latest.error)throw latest.error;const base=assemble(u.id,baseRes),profile=mergeSafe(base,latest.data?.snapshot||{}),revision=Number(latest.data?.revision||0);profile.revision=revision;profile.schemaVersion=37;return J({ok:true,profile,revision,role});
    }
    ...
    await syncSettings(db,u.id,incoming);await syncLoadouts(db,u.id,incoming.loadouts);
