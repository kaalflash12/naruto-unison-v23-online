...
function loadRoster(){
  ...
  const team=starters.map(slug=>roster.find(c=>c.slug===slug));
...
function payPlan(ch,cost){
  const t=Object.fromEntries(chakraTypes().map(k=>[k,Number(ch?.[k]||0)]));let wild=0;
  for(const c of(cost||[])){if(c==='Rand'){wild++;continue}if(!chakraTypes().includes(c)||t[c]<=0)return null;t[c]--}
