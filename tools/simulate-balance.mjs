...
function tick(teams,skillStats){for(const team of teams)for(const f of team){if(f.hp>0&&f.dot&&f.dotTurns>0){const h=hit(f,f.dot);if(f.dotSource&&skillStats.has(f.dotSource))skillStats.get(f.dotSource).dotDamage+=h.hp;f.dotTurns--;if(f.dotTurns<=0){f.dot=0;f.dotSource=null}}for(const s of f.skills)if(s.cd)s.cd--}}
function simulate(charsA,charsB,{first='A',policyA='balanced',policyB='balanced',seed='0',skillStats}){const rng=rngFrom(seed),A=charsA.map(cloneChar),B=charsB.map(cloneChar),chA=gain(emptyCh(),6,rng),chB=gain(emptyCh(),6,rng);let turn=1;for(;turn<=MAX_TURNS;turn++){const step=(own,enemy,ch,policy)=>{const acts=planActions(own,enemy,ch,policy,rng);performActions(own,enemy,acts,rng,skillStats)};if(first==='A'){step(A,B,chA,policyA);expireDefenseAfterOpponentPhase(B);if(!alive(B).length)return{winner:'A',turns:turn};step(B,A,chB,policyB);expireDefenseAfterOpponentPhase(A)}else{step(B,A,chB,policyB);expireDefenseAfterOpponentPhase(A);if(!alive(A).length)return{winner:'B',turns:turn};step(A,B,chA,policyA);expireDefenseAfterOpponentPhase(B)}if(!alive(A).length)return{winner:'B',turns:turn};if(!alive(B).length)return{winner:'A',turns:turn};tick([A,B],skillStats);if(!alive(A).length&&!alive(B).length)return{winner:'draw',turns:turn};if(!alive(A).length)return{winner:'B',turns:turn};if(!alive(B).length)return{winner:'A',turns:turn};gain(chA,3,rng);gain(chB,3,rng)}return{winner:'draw',turns:MAX_TURNS}}
