...
outer:for(let i=0;i<chars.length;i++)for(let j=i+1;j<chars.length;j++){
 ...
 for(const policy of POLICIES)for(let s=0;s<SEEDS;s++)for(const first of['A','B']){
  ...
  if(r.winner==='A')m.aWins++;else if(r.winner==='B')m.bWins++;else m.draws++;
  for(const [slug,outcome,met,hpd]of[[A.slug,r.winner==='A'?'win':r.winner==='B'?'loss':'draw',r.metricsA,r.hpA-r.hpB],[B.slug,r.winner==='B'?'win':r.winner==='A'?'loss':'draw',r.metricsB,r.hpB-r.hpA]]){
   const st=stats.get(slug);st.matches++;st[outcome==='win'?'wins':outcome==='loss'?'losses':'draws']++;st.turns+=r.turn;st.hpDiff+=hpd;st.damage+=met?.damage||0;st.heal+=met?.heal||0;st.defense+=met?.defense||0;st.control+=met?.control||0;st.chakra+=met?.chakra||0;st.actions+=met?.actions||0;for(let k=0;k<4;k++)st.uses[k]+=met?.uses?.[k]||0
  }
 }
